...
- **Kapı V2 — kod hazır, kurulum bekliyor:** [docs/KAPI_TASARIM.md](docs/KAPI_TASARIM.md). Pilot için Bülent Plesk'te `giris.saldir.tr` alt alan adını (Node.js açık) oluşturmalı → `deploy/kapi-kurulum.sh`. **DİKKAT:** `plesk sbin mail_auth_view` tüm posta parolalarını düz metin verir — teşhis çıktısı asla ekrana/loga basılmaz (2026-08-09'da iki hesabın parolası oturum kaydına düştü, Bülent'e bildirildi).
