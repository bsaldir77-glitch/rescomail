...
- **YAPILDI (2026-08-09): Resco Mail Yönetim V1 CANLI — `mailprovider.rescopos.com`.** Tasarım: [docs/YONETIM_TASARIM.md](docs/YONETIM_TASARIM.md). Kapsam: hesap listesi (sogo_users_view), hesap aç/şifre sıfırla/sil (Plesk API `cli/mail/call`), OTP telefon/anahtar, **şifreli Bağlantılar kasası** (AES-256-GCM, BAGLANTI_KEY .env'de), NetGSM Test SMS (**doğrulandı**: gerçek SMS ulaştı), işlem kaydı. Passenger/Plesk Node: AppRoot `rescomail/yonetim`, DocRoot `.../public`. **Kod deploy'u sonrası "Restart App" şart** (Passenger eski süreci tutar — test ucu 404 vakası). Uygulama klonu: subdomain dizininde `rescomail/`; kurulum/güncelleme: `deploy/yonetim-kurulum.sh` (idempotent).
- **Kapı KURULDU (2026-08-09) — TEK ADRES `webmail.rescopos.com`:** [docs/KAPI_TASARIM.md](docs/KAPI_TASARIM.md). Giriş kutusu SOGo giriş sayfasına `theme.js` ile iner (restart gerekmez), doğrulama panelin `/api/kapi/*` uçlarında, parola köprüsü root'ta (sudo, tek komut). Kullanıcı açma: panel → OTP Ayarları. Kullanılmayan: `kapi/` klasörü + `giris.saldir.tr` (silme onayı bekliyor). **DİKKAT:** `plesk sbin mail_auth_view` tüm posta parolalarını düz metin verir — teşhis çıktısı asla ekrana/loga basılmaz (2026-08-09'da iki hesabın parolası oturum kaydına düştü, Bülent'e bildirildi).
- ~~Faz önerisi — SMS-OTP parolasız giriş~~ → Kapı V2 olarak tasarlandı (yukarı). Eski not: e-posta adresi + telefona 6 haneli kod, şifre hiç girilmez/dolaşmaz. SOGo'da yerleşik YOK (yerleşik olan yalnız TOTP-uygulama 2FA). Kurulabilir yol: önde OTP portalı (NetGSM, rescopos api-hub deseni) + `SOGoTrustProxyAuthentication` + IMAP master-user — ciddi güvenlik mühendisliği ister (trust başlığı sızarsa tüm kutular açılır); tasarım dokümanı yazılıp onaylanmadan BAŞLANMAZ.
- **Yönetim girişi "tek kod" (2026-08-09, Bülent kararı):** SMS'li giriş açıkken parola sorulmaz — e-posta + telefona gelen 6 haneli kod (5 dk geçerli, 5 deneme, 60 sn yeniden-gönderim freni). **Acil kurtarma** (yanlış telefon/NetGSM arızası kilitlediyse, yalnız Bülent onayıyla): `rescomail_db`'de `UPDATE yoneticiler SET sms_giris=false;` → parola girişi geri gelir.
